...
#SBATCH --gpus=1
#SBATCH -t 1-00:00:00
#SBATCH -J t2v_phenotype_pr_ARG1
...
# -------------------------------
# PHENOTYPE SETUP FOR PROLIFERATION
# -------------------------------
# Set phenotype values based on condition
# Format: "cell_count,proliferation,migration,cell_death"
if [[ "$CONDITION" == "HIGH" ]]; then
  # High proliferation rate (1.0), other values default (0.5)
  PHENOTYPES="0.5,1.0,0.5,0.5"
else
  # Low proliferation rate (0.0), other values default (0.5)
  PHENOTYPES="0.5,0.0,0.5,0.5"
fi
...
OUTDIR="../../../data/generated/final_evals/phenotype_pr"
mkdir -p "$OUTDIR"
...
echo "All done! Generated $NUM_VIDEOS phenotype-conditioned videos for pr-$CONDITION in $PHENO_DIR" 